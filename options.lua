...
local function RegisterDefaultOption(key, value)
    if RAVENOUS_data.options[key] == nil then
        RAVENOUS_data.options[key] = value
    end
end
...
function ns:SetDefaultOptions()
    if RAVENOUS_data == nil then
        RAVENOUS_data = {}
    end
    if RAVENOUS_data.options == nil then
        RAVENOUS_data.options = {}
    end
    ...
    for _, Default in pairs(L.Defaults) do
        ...
        if type(defaultValue) == "boolean" then
            ...
            Checkbox.SetValue = function(self)
                self:SetChecked(RAVENOUS_data.options[Default.var])
            end

            Checkbox:SetScript("OnClick", function(self)
                PlaySound(SOUNDKIT.IG_MAINMENU_OPTION_CHECKBOX_ON)
                self.restart = not self.restart
                RAVENOUS_data.options[Default.var] = self:GetChecked()
